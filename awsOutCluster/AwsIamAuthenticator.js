...
const awsIamAuthenticator = (region, ttl, profile) => ({ sasl, host, port, logger, saslAuthenticate }) => {
  const INT32_SIZE = 4
  ...
  return {
    authenticate: async () => {
      ...
      const payloadFactory = new AuthenticationPayloadCreator({ region, ttl, profile })
